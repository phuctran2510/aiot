...
export default function Theory() {
  const [ch, setCh] = useState(THEORY[0])
      ...
      <div style={{display:'grid',gridTemplateColumns:'185px 1fr',gap:'1.1rem',alignItems:'start'}}>
        ...
        <div className="fu">
          <div className="card" style={{padding:'.95rem 1.1rem',marginBottom:'.75rem',background:`${ch.color}06`,borderColor:`${ch.color}22`}}>
            <span className="badge" style={{background:`${ch.color}12`,color:ch.color,border:`1px solid ${ch.color}28`,marginBottom:'.4rem'}}>{ch.title}</span>
          ...
          {ch.sections.length > 1 && (
            <div className="tabs">
              {ch.sections.map(s => (
                <button key={s.id} className={`tab${sc.id===s.id?' on':''}`} onClick={() => { setSc(s); window.scrollTo(0,0) }}>{s.title}</button>
              ))}
            </div>
          )}
